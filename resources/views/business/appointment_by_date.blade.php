              ...
              <tbody>
                <?php $i=1; ?>
                  @foreach ($book_slot as $key => $slot)
                    <?php 
                    $appointmentCount = \App\Models\Booking_add_cart_time_order::where('slot_id',$slot->id)->whereDate('date', $date)->count();  
                    ...
                    <tr>
                      ...
                      @if($slot->limit_per_slot == $appointmentCount)
                        <td class="text-center"><a href="" data-toggle="modal" data-target="#myBooking_{{$slot->id}}"><?php echo "BOOKED"; ?></a></td>
                              ...
                              <div class="modal-content">
                                ...
                                <div class="modal-body">  
                                    <?php  $booking_data = \App\Models\Booking_add_cart_time_order::where('slot_id',$slot->id)
                                                            ->whereDate('booking_add_cart_time_order.date', $date)
                      ...
                      @else
                      <?php $appointmentCount = \App\Models\Booking_add_cart_time_order::where('slot_id',$slot->id)->whereDate('date', $date)->count();  ?>
                        @if($appointmentCount == 0)
                          <td class="text-center"><a href="{{ url('book_appointment',['id'=> $slot->id,'date'=>$date]) }}"><?php echo "AVAILABLE"; ?></a></td>
                        @else
